...
A simpler implementation will make use of the provided Datasonnet mix-in class instead of a custom one. In this case, the `PolymorphicTypes` header must be set, and optionally the `PolymorphicTypeIdProperty`, e.g.:
