...
The `Formats` library contains `read`, and `write` functions for parsng and generating data in other data formats, e.g. XML or CSV. The XML and CSV plugins are built into the standard DataSonnet distributions while other formats can be supported by implementing `DataFormatPlugin` interface.
...
*Example*
------------------------
DS.Formats.write(payload.embeddedXMLValue, "application/csv")
------------------------
...
DS.Formats.write(payload.embeddedXMLValue, "application/xml", params);
------------------------
